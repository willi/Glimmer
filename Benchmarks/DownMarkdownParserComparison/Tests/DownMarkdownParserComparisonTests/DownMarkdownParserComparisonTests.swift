...
final class DownMarkdownParserComparisonTests: XCTestCase {
    func testDownMarkdownParserComparison() throws {
        guard Self.isBenchmarkEnabled else {
            throw XCTSkip(
                "set GLIMMER_COMPARE_MARKDOWN_PARSERS=1 or touch " +
                "/tmp/glimmer-run-markdown-parser-comparison to run Down comparison benchmarks"
            )
        }
        ...
        print("[COMPARE] Down is isolated because Down's libcmark conflicts with swift-cmark in one build graph.")
    ...
    private static func makeBenchmarkCases() -> [BenchmarkCase] {
        ...
        return [
            ...
            BenchmarkCase(name: "Down", operation: "HTML", notes: "cmark CommonMark toHTML") { markdown in
                try Down(markdownString: markdown).toHTML().utf8.count
            },
        ]
    }
    ...
    private static func printResults(_ results: [BenchmarkResult], corpusOrder: [String]) {
        for corpus in corpusOrder {
            ...
            for operation in ["AST/tree", "HTML", "Attributed"] {
