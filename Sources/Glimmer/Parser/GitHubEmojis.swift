...
public enum GitHubEmojis {
    ...
    /// GitHub emoji image URLs for rendering as images
    /// This is a small sample - in production, this would include all emojis
    public static let emojiUrls: [String: String] = [
    ...
    /// Process emoji shortcodes in markdown text, converting them to Unicode emojis or image markdown
    /// - Parameter text: The input text containing emoji shortcodes
    /// - Returns: Text with emoji shortcodes replaced by actual emojis or image markdown
    public static func processEmojiShortcodes(_ text: String) -> String {
        let regex = emojiShortcodeRegex
        ...
        for match in matches {
            ...
            if let unicodeEmoji = emojiMap[shortcode], !unicodeEmoji.hasPrefix(":") {
                ...
                // Replace with image markdown for GitHub-specific emojis
                let imageMarkdown = "![:\(shortcode):](\(imageUrl))"
                result.replaceSubrange(matchRange, with: imageMarkdown)
            }
            // If neither Unicode nor image URL is found, leave the shortcode unchanged
        }
    ...
    // MARK: - Accessors (Lazy URL Map)
    /// Returns the URL string for an emoji shortcode, honoring lazy map if enabled.
    public static func emojiURL(for name: String) -> String? {
        if useLazyEmojiURLMap {
    ...
    public static func emojiURLMap() -> [String: String] {
        if useLazyEmojiURLMap, let map = emojiURLMapLazy() {
            return map
        }
        return emojiUrls
    }
    ...
    private static func emojiURLMapLazy() -> [String: String]? {
        ...
        if let url = Bundle.module.url(forResource: "emoji_urls", withExtension: "json") {
            do {
                ...
                #if DEBUG
                print("[GitHubEmojis] Failed to load emoji_urls.json: \(error)")
                #endif
            }
        }
        #endif
